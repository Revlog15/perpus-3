...
export async function renderHistory(target) {
  ...
  try {
    ...
    if (!idUser) {
      target.innerHTML = '<div class="alert alert-warning">Silakan login untuk melihat riwayat</div>';
      return;
    }
    ...
    if ((!history.activeLoans?.length) && (!history.returnHistory?.length)) {
      target.innerHTML = '<div class="alert alert-info">Belum ada riwayat peminjaman</div>';
      return;
    }
    ...
    if (history.returnHistory?.length) {
      history.returnHistory.forEach(ret => {
        ...
        allRecords.push({
          ...
          status: 'Dikembalikan',
          info: ret.denda > 0 ? `Denda: Rp ${ret.denda.toLocaleString()}` : 'Tepat waktu',
          type: 'return'
        });
      });
    }
    ...
    allRecords.forEach(record => {
      ...
      tr.innerHTML = `
        <td>${record.date}</td>
        <td>${record.book}</td>
        <td><span class="badge bg-${record.type === 'loan' ? 'primary' : 'success'}">${record.status}</span></td>
        <td>${record.info}</td>
      `;
    ...
    target.innerHTML = '<div class="alert alert-danger">Gagal memuat riwayat peminjaman</div>';
  }
}
